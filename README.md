...
**3. Explore the graph.** Open the **[live explorer](https://yue-jiang.github.io/pharma-space-public/)** in a browser, or download [`graph/explorer.html`](graph/explorer.html) to use it offline. It is one self-contained file with no server. The Commercial layer shows marketed and legacy assets, with drug dot size driven by sales. The Bets layer opens with a portfolio-similarity map across all validated fields, then lets you drill into one field's active pipeline assets. Company dot size is driven by a validated commitment category. Click any node for its evidence and context. Space expands the neighbourhood one hop, and shift+space retracts.
